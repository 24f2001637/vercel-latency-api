...
# ============================================================
# TELEMETRY DATA — already embedded, no need to touch
# ============================================================
TELEMETRY_DATA = [
  {
    "region": "apac",
...
# Enable CORS so ANY website can POST to this endpoint
app.add_middleware(
    CORSMiddleware,
...
def calculate_p95(values):
    """Calculate the 95th percentile using nearest-rank method."""
    if not values:
        return 0
...
async def analyze(request: RequestBody):
    """
    Accepts: {"regions": ["apac", "emea"], "threshold_ms": 187}
    Returns: per-region metrics
    """
    results = {}

    for region in request.regions:
        # Grab only the records for this region
        region_records = [
            r for r in TELEMETRY_DATA
            if r.get(FIELD_REGION) == region
        ]
